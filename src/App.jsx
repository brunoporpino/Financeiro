...
moradia: {
label: “🏠 Moradia”, color: “#9b59b6”,
items: [
{ name: “Aluguel”, amt: 1500 },
{ name: “Condomínio”, amt: 499.53 },
{ name: “Cosern (energia)”, amt: 320 },
{ name: “Ultragaz”, amt: 35 },
{ name: “IPTU”, amt: 200.60 },
]
},
saude: {
label: “💊 Saúde”, color: “#1abc9c”,
items: [
{ name: “Unimed”, amt: 707.02, note: “vence dia 15” },
{ name: “Terapia (4x/mês)”, amt: 1040 },
{ name: “Prudential”, amt: 489.49, note: “⚠️ cancelar quando possível” },
]
},
carro: {
label: “🚗 Carro”, color: “#3498db”,
items: [
{ name: “Financiamento Carro (Inter)”, amt: 1800.90, note: “vence dia 25” },
{ name: “Seguro Carro (Santander)”, amt: 230.20, note: “débito automático” },
{ name: “IPVA (10x)”, amt: 408.66, note: “fev–nov” },
]
},
financiamentos: {
label: “💳 Financiamentos”, color: “#c8f135”,
items: [
...
assinaturas: {
label: “🔄 Assinaturas”, color: “#35c8f1”,
items: [
...
const TOTAL_FIXED = Object.values(COMMIT_SECTIONS).reduce((s, sec) =>
s + sec.items.reduce((ss, i) => ss + i.amt, 0), 0);
...
const [commitTab, setCommitTab] = useState(“moradia”);
...
(async () => {
try {
const r = await window.storage.get(“financas_v3”);
...
setSaving(true);
try { await window.storage.set(“financas_v3”, JSON.stringify({entries:e,projects:p})); } catch {}
setSaving(false);
}, []);
...
```
  ...
  <div style={{padding:"20px 20px 0",borderBottom:`1px solid ${S.border}`}}>
    ...
    <div style={{display:"flex"}}>
      {[["dashboard","📊"],["add","＋"],["list","☰"],["commit","🔒"]].map(([v,l])=>(
  ...
  {view==="dashboard" && (
    <div className="su" style={{padding:"12px 20px 0"}}>
      ...
      <div style={{background:S.surface2,border:`1px solid ${S.danger}44`,borderRadius:12,padding:16,marginBottom:20,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div>
          <div style={{fontSize:10,color:S.muted,textTransform:"uppercase",letterSpacing:1}}>Comprometido/mês</div>
          <div style={{fontSize:10,color:S.muted,marginTop:2}}>antes de qualquer gasto · ver 🔒</div>
        </div>
        <div style={{fontFamily:"Syne",fontWeight:800,fontSize:22,color:S.danger}}>R$ {fmt(TOTAL_FIXED)}</div>
      </div>
    </div>
  )}
      ...
      {!projView?(
        <>
          <div style={{display:"flex",gap:8,marginBottom:14}}>
            {["entrada","saída"].map(t=>(
              <button key={t} onClick={()=>setType(t)} style={{flex:1,padding:"10px",borderRadius:8,cursor:"pointer",background:type===t?(t==="entrada"?S.accent:S.danger):"transparent",border:`1px solid ${type===t?(t==="entrada"?S.accent:S.danger):S.border}`,color:type===t?"#000":S.muted,fontFamily:"'IBM Plex Mono'",fontWeight:700,fontSize:13}}>
        ...
        <>
          <div style={{fontSize:11,color:S.muted,marginBottom:16,lineHeight:1.7,background:S.surface,border:`1px solid ${S.border}`,borderRadius:8,padding:12}}>Registre projetos de identidade visual. Marque ✓ quando o pagamento entrar.</div>
          <div style={{marginBottom:12}}>
            <Label>Cliente</Label>
  ...
  {view==="commit" && (
    <div className="su" style={{padding:"16px 20px 0"}}>
      <div style={{background:S.surface2,border:`1px solid ${S.danger}44`,borderRadius:12,padding:16,marginBottom:16,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div>
          <div style={{fontSize:10,color:S.muted,textTransform:"uppercase",letterSpacing:1}}>Total comprometido/mês</div>
          <div style={{fontSize:10,color:S.muted,marginTop:2}}>antes de qualquer gasto variável</div>
        </div>
        <div style={{fontFamily:"Syne",fontWeight:800,fontSize:22,color:S.danger}}>R$ {fmt(TOTAL_FIXED)}</div>
      </div>

      <div style={{display:"flex",gap:6,overflowX:"auto",marginBottom:14,paddingBottom:2}}>
        {Object.entries(COMMIT_SECTIONS).map(([key,sec])=>(
          <button key={key} onClick={()=>setCommitTab(key)} style={{flexShrink:0,padding:"5px 10px",borderRadius:4,cursor:"pointer",background:commitTab===key?`${sec.color}20`:S.surface,border:`1px solid ${commitTab===key?sec.color:S.border}`,color:commitTab===key?sec.color:S.muted,fontFamily:"'IBM Plex Mono'",fontSize:10,fontWeight:600}}>
            {sec.label.split(" ")[0]}
          </button>
        ))}
      </div>

      {Object.entries(COMMIT_SECTIONS).map(([key,sec])=>commitTab===key&&(
        <div key={key}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
            <div style={{fontFamily:"Syne",fontWeight:700,fontSize:14}}>{sec.label}</div>
            <div style={{fontFamily:"Syne",fontWeight:700,fontSize:14,color:sec.color}}>R$ {fmt(sec.items.reduce((s,i)=>s+i.amt,0))}</div>
          </div>
          <div style={{background:S.surface,border:`1px solid ${S.border}`,borderRadius:10,overflow:"hidden"}}>
            {sec.items.map((item,i)=>(
              <div key={i} style={{padding:"12px 14px",borderBottom:i<sec.items.length-1?`1px solid ${S.border}`:"none"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                  <div style={{flex:1}}>
                    <div style={{fontSize:12}}>{item.name}</div>
                    {item.note&&<div style={{fontSize:10,color:item.note.includes("⚠️")?S.warn:S.muted,marginTop:2}}>{item.note}</div>}
                    {item.remaining&&<div style={{fontSize:10,color:S.muted,marginTop:2}}>{item.remaining}x restantes · total R$ {fmt(item.amt*item.remaining)}</div>}
                  </div>
                  <div style={{fontFamily:"Syne",fontWeight:700,fontSize:13,color:sec.color,flexShrink:0,marginLeft:8}}>R$ {fmt(item.amt)}</div>
                </div>
              </div>
            ))}
          </div>
